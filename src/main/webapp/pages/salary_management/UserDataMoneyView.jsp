...
<div class="x-nav" style="margin-top: 10px;margin-left: 10px;">
    ...
    <%--            <span lay-separator>/</span>--%>
                <span class="spanTitle"><a>员工列表</a> </span>
            </span>
...
<div style="margin-left: 20px;margin-top: 20px;">
    <%--职称表格--%>
    <table class="layui-hide" id="UserDataMoneyList"  lay-filter="demo"></table>
...
<script>
    ...
    layui.use(['layer', 'table', 'element'], function(){
        ...
        var tableIns = table.render({
            elem: '#UserDataMoneyList'
            ,height: 540 //设置表的高度
            ,url: '${pageContext.request.contextPath}/getAllUserDataMoney' //数据接口  //（！）
            ,title: '员工表'
            ,page: true //开启分页
            ,limits: [5,10,15,20]
            ,limit: 5
            ...
            ,cols: [[ //表头
                ...
                {field: 'userId', width: 182, align:'center',  title: 'ID', sort: true,
                    style: 'font-size:18px;'}  // 用户编号
                ,{field: 'userPhone',width: 246, align:'center', title: '员工手机号',
                    style: 'font-size:18px;'}
                ,{field: 'userName',width: 166, align:'center',title: '员工姓名',
                    style: 'font-size:18px;'}
                ,{field: 'departmentName',width: 192, align:'center',title: '部门名称',
                    style: 'font-size:18px;'}
                ,{field: 'positionName',width: 194, align:'center',title: '岗位名称',
                    style: 'font-size:18px;'}
                ,{field: 'positionMoney',width: 217, align:'center',title: '岗位工资',
                    style: 'font-size:18px;'}
                ,{field: 'workTitleName',width: 205, align:'center',title: '职称名称',
                    style: 'font-size:18px;'}
                ,{field: 'workTitleMoney',width: 223, align:'center',title: '职称工资',
                    style: 'font-size:18px;'}
                ,{field: 'workAge',width: 211, align:'center',title: '工龄',
                    style: 'font-size:18px;'}
                ,{field: 'baseAgeMoney',width: 260, align:'center',title: '工龄基础工资',
                    style: 'font-size:18px;'}
        ...
        table.on('tool(demo)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                ...
                var index = layer.open({
                    ...
                    //加载编辑页面的弹出层
                    content: 'UserDataMoneyDetail.jsp?' + 'userId=' + data.userId
                        + '&' +'userPhone=' + data.userPhone + '&' + 'userName=' + data.userName
                        + '&' +'departmentName=' + data.departmentName + '&' + 'positionName=' + data.positionName
                        + '&' +'positionMoney=' + data.positionMoney + '&' + 'workTitleName=' + data.workTitleName
                        + '&' +'workTitleMoney=' + data.workTitleMoney + '&' + 'workAge=' + data.workAge
                        + '&' +'baseAgeMoney=' + data.baseAgeMoney + '&' + 'userPhoto=' + data.userPhoto,

                    btn: '关闭',
                    skin: 'editLayer',   // 自定义样式
                    yes:function(){
                        tableIns.reload("demo",{
                            url:'${pageContext.request.contextPath}/getAllUserDataMoney'
                        });
                        layer.closeAll();
                    },
                    btnAlign: 'c', //按钮居中
