...
public class GetSaleTaskByUserIdServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        System.out.println("============进入 GetSaleTaskByUserIdServlet============");

        UserAccount userAccount = (UserAccount) request.getSession().getAttribute("userAccount");

        SaleTaskService service = new SaleTaskServiceImpl();
        List<SaleTask> saleTaskList = service.getSaleTaskByUserId(userAccount.getUserId());
