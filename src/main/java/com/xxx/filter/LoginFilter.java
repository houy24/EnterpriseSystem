...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(url.equals(contextPath + "/login.jsp") || url.equals(contextPath + "/")
            ...
            System.out.println("登录页，放行 => " + url);
        }else if(url.endsWith(".css") || url.endsWith(".js") || url.endsWith(".gif") || url.endsWith(".png")  ){ // .css .js 结尾
            //如果发现是css或者js文件，直接放行
            filterChain.doFilter(servletRequest, servletResponse);
            System.out.println(".css .js .git .png 放行 => " + url);
        }else {
            ...
            if ("".equals(session.getAttribute("userAccount")) || session.getAttribute("userData") == null) {
                ...
                out.write("<script type=\"text/javascript\" charset=\"utf-8\" >alert('请先登录！');" +
                        "window.parent.location.href=\"" + contextPath + "/\";</script>");

                System.out.println("请先登录！ => " + url);
