...
<%--岗位列表--%>
    ...
    <div class="x-nav" style="margin-top: 10px;margin-left: 10px;">
        ...
        <%--            <span lay-separator>/</span>--%>
                    <span class="spanTitle"><a>岗位列表</a> </span>
                </span>
    ...
    <div style="margin-left: 20px;margin-top: 20px;">
        <%--岗位表格--%>
        <table class="layui-hide" id="PositionList"  lay-filter="demo"></table>
    ...
    <script>
        ...
        layui.use(['layer', 'table', 'element'], function(){
            ...
            var tableIns = table.render({
                elem: '#PositionList'
                ,height: 540 //设置表的高度
                ,url: '${pageContext.request.contextPath}/getAllPosition' //数据接口
                ,title: '岗位表'
                ,page: true //开启分页
                ,limits: [5,10,15,20]
                ,limit: 5
                ...
                ,cols: [[ //表头
                    ...
                    //职位表（职位编号, 职位名称,岗位工资，职位描述）
                    //position (positionId,positionName,positionMoney,positionDesrciption)

                    {type:'checkbox'},  //复选框

                    // {field: 'positionId', width: 50, align: "center", title: '岗位编号'}, // 不显示
                    {field: 'positionName', width: 293, align:'center',  title: '岗位名称', sort: true,
                        style: 'font-size:18px;'} //岗位编号
                    ,{field: 'positionDesrciption',width:386, align:'center', title: '岗位描述',
                        style: 'font-size:18px;'}
                    ,{field: 'positionMoney',width:281, align:'center',title: '岗位工资',
                        style: 'font-size:18px;'}
            ...
            table.on('toolbar(demo)', function(obj){
                ...
                if (layEvent === 'add') { // 增加
                    ...
                    var index = layer.open({
                        ...
                        offset: '9%',
                        //加载编辑页面的弹出层
                        content: 'PositionAdd.jsp',
                        btn: '关闭',
                        skin: 'editLayer',   // 自定义样式
                        yes:function(){     // 点关闭按钮的话，刷新页面
                            tableIns.reload("demo",{
                                url:'${pageContext.request.contextPath}/getAllPosition'
                            });
                            layer.closeAll();
                        },
                        btnAlign: 'c', //按钮居中
            ...
            table.on('tool(demo)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'del')
                {
                    ...
                    var index = layer.open({
                        ...
                        offset: '9%',
                        //加载编辑页面的弹出层
                        content: 'PositionEdit.jsp?' + 'positionId=' + data.positionId
                            + '&' +'positionName=' + data.positionName
                            + '&' + 'positionMoney=' + data.positionMoney
                            + '&' + 'positionDesrciption=' + data.positionDesrciption,
                        btn: '关闭',
                        skin: 'editLayer',   // 自定义样式
                        yes:function(){
                            tableIns.reload("demo",{
